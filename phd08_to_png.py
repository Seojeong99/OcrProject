...
def txt_to_png(file_orig_path, file_full_path, width, height, sigma):
    file_orig_path = file_orig_path[:-4]  #가
    save_dir = 'phd08_png_results1/' + file_orig_path
    if not os.path.exists('phd08_png_results1/' + file_orig_path):
    ...
    with open(file_full_path, 'r') as lines:
        ...
        for line in lines:
            if font_start_checker(line):  # endl
                ...
                fig.savefig('phd08_png_results1/' + file_orig_path +
                            '/' + font_name + '.png', dpi=1)
                plt.close(fig)
...
def main():
    ...
    print("INFO:: all files converted to png, results in phd08_png_results1/")
